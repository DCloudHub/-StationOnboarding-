...
def get_gsheet_client():
    if "gcp_service_account" not in st.secrets:
        raise RuntimeError("Missing Google service account in Streamlit secrets.")
...
if submitted:
    if not name:
        ...
        spreadsheet_id = extract_spreadsheet_id(
            spreadsheet_input or st.secrets.get("spreadsheet_id", "")
        )
        if spreadsheet_id:
            try:
...
if st.button("Import to Google Sheet"):
    try:
        ...
        spreadsheet_id = extract_spreadsheet_id(
            spreadsheet_input2 or st.secrets.get("spreadsheet_id", "")
        )
