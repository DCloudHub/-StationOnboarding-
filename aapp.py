...
if 'auto_location_captured' not in st.session_state:
    st.session_state.auto_location_captured = False
...
# JavaScript for GPS location capture
def get_gps_location_js():
    """JavaScript to get GPS location"""
    return """
    <script>
...
if st.session_state.admin_authenticated and st.session_state.view_submissions:
    ...
    # Step 4: Location Verification - WITH GPS BUTTON
    elif st.session_state.current_step == 4:
        st.markdown("### Step 4: Location Verification")
        
        # Inject JavaScript for GPS
        st.components.v1.html(get_gps_location_js(), height=0)
        ...
        st.markdown("---")
        st.markdown("### 📍 Capture GPS Location Automatically")
        ...
        with col_gps1:
            ...
            **Get precise coordinates automatically:**
            ...
            - Coordinates will auto-fill below
            
            *Note: You already gave consent for GPS capture in Step 1*
            """)
        
        with col_gps2:
            # GPS Capture Button
            if st.button("📍 Get My GPS Location", type="primary", use_container_width=True):
                # Inject JavaScript to trigger GPS capture
                trigger_js = """
                <script>
                getGPSLocation();
                ...
                function checkForGPSData() {
                    const elem = document.getElementById('gpsLocationData');
                    if (elem) {
                        const data = JSON.parse(elem.innerText);
                        // Send to Streamlit via URL parameters
                        window.location.href = window.location.href.split('?')[0] + '?gps=' + encodeURIComponent(elem.innerText);
                    } else {
                        setTimeout(checkForGPSData, 500);
                    }
                }
                checkForGPSData();
                </script>
        ...
        # Check for GPS data in URL
        query_params = st.query_params
        if 'gps' in query_params:
            try:
                gps_data = json.loads(query_params['gps'])
                
                if 'error' in gps_data and gps_data['error']:
                    st.error(f"GPS Error: {gps_data.get('message', 'Could not get location')}")
                else:
                    latitude_val = gps_data.get('latitude')
                    longitude_val = gps_data.get('longitude')
                    
                    if latitude_val and longitude_val:
                        st.session_state.auto_location_captured = True
                        st.success(f"✅ GPS Location Captured! Accuracy: ±{gps_data.get('accuracy', 'Unknown')} meters")
                        ...
                        if st.session_state.photo_metadata:
                            st.session_state.photo_metadata['latitude'] = latitude_val
                            st.session_state.photo_metadata['longitude'] = longitude_val
                        
                        # Clear the query parameter
                        del st.query_params['gps']
                        st.rerun()
        ...
        # Pre-fill values from GPS if available
        if st.session_state.location_data and st.session_state.auto_location_captured:
            default_lat = st.session_state.location_data.get('latitude', 0.0)
            default_lon = st.session_state.location_data.get('longitude', 0.0)
        else:
            # Get default values from photo metadata
            default_lat = 0.0
            default_lon = 0.0
            ...
            if st.button("Next: Review & Submit →", type="primary"):
                if latitude != 0.0 and longitude != 0.0:
                    ...
                    st.session_state.location_data = {
                        ...
                        'longitude': longitude,
                        'timestamp': datetime.now(NIGERIA_TZ).isoformat(),
                        'source': 'gps' if st.session_state.auto_location_captured else 'manual'
                    }
    ...
    # Step 5: Review & Submit
        ...
        if all([st.session_state.client_data, st.session_state.photo_captured, st.session_state.location_data]):
            ...
            with col_s2:
                ...
                location_source = loc.get('source', 'manual')
                source_icon = "📍" if location_source == 'gps' else "✏️"
                st.write(f"**Coordinates:** {loc['latitude']:.6f}, {loc['longitude']:.6f}")
                st.write(f"**Source:** {source_icon} {'GPS Auto-capture' if location_source == 'gps' else 'Manual Entry'}")
                ...
                if st.button("Submit Station Registration", type="primary", use_container_width=True):
                    ...
                    final_data = {
                        **st.session_state.client_data,
                        ...
                        'location_source': st.session_state.location_data.get('source', 'manual'),
                    ...
                    if save_submission_to_db(final_data, photo_bytes, st.session_state.photo_metadata):
                        ...
                        **Photo Metadata:**
                        ...
                        - **Source:** {'📍 GPS Auto-capture' if final_data['location_source'] == 'gps' else '✏️ Manual Entry'}
                        ...
                        **Important:** Your station photo with embedded metadata serves as official documentation.
                        ...
                        summary = {
                            ...
                            'location_source': final_data['location_source'],
